...
int setup_uinput_device() {
	...
	strncpy(uinp.name, "NES-to-Keyboard device", strlen("NES-to-Keyboard device"));
	uinp.id.version = 4;
...
int main(int argc, char *argv[]) {
	...
	NEScontrollers controllers;
	controllers.clock   = RPI_GPIO_P1_07;
	controllers.latch   = RPI_GPIO_P1_11;
	controllers.data1   = RPI_GPIO_P1_13;
	controllers.data2   = RPI_GPIO_P1_15;
	...
	while ( 1 ) {
		...
		/* key events for first controller */
        	processBtn(buttons.buttons1,NES_A,KEY_X,uinp_fd);
        	processBtn(buttons.buttons1,NES_B,KEY_Z,uinp_fd);
        	processBtn(buttons.buttons1,NES_SELECT,KEY_RIGHTSHIFT,uinp_fd);
        	processBtn(buttons.buttons1,NES_START,KEY_ENTER,uinp_fd);
        	processBtn(buttons.buttons1,NES_LEFT,KEY_LEFT,uinp_fd);
        	processBtn(buttons.buttons1,NES_RIGHT,KEY_RIGHT,uinp_fd);
        	processBtn(buttons.buttons1,NES_UP,KEY_UP,uinp_fd);
        	processBtn(buttons.buttons1,NES_DOWN,KEY_DOWN,uinp_fd);

		/* key events for second controller */
        	processBtn(buttons.buttons2,NES_A,KEY_E,uinp_fd);
        	processBtn(buttons.buttons2,NES_B,KEY_R,uinp_fd);
        	processBtn(buttons.buttons2,NES_SELECT,KEY_O,uinp_fd);
        	processBtn(buttons.buttons2,NES_START,KEY_P,uinp_fd);
        	processBtn(buttons.buttons2,NES_LEFT,KEY_C,uinp_fd);
        	processBtn(buttons.buttons2,NES_RIGHT,KEY_B,uinp_fd);
        	processBtn(buttons.buttons2,NES_UP,KEY_F,uinp_fd);
        	processBtn(buttons.buttons2,NES_DOWN,KEY_V,uinp_fd);
